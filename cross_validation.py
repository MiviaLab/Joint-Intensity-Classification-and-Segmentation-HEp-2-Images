...
def predict(img, model, preprocess, device):
    ...
    img = img.to(device)
    with torch.no_grad():
        seg, intens = model(img)  # send through model/network
    return seg, intens
...
#unica funzione da cambiare in base alla dimensione della patch, allo stride e alla sovrapposizione
...
class Trainer:
    ...
    def _train(self):
        ...
        for i, (x, y) in batch_iter:
            ...
            intens = y[1].to(self.device)
            
            self.optimizer.zero_grad(set_to_none=True)  # zerograd the parameters
            out = self.model(input)  # one forward pass
            loss = self.criterion(out, mask, intens)  # calculate loss
            l += loss
        ...
        dice_loss, bce_loss = self.criterion.get_losses(c)
        ...
        model_name = 'hep2_model_cross_v_' + self.fold + '_suff_5.pt'
...
for i in range(5):
    ...
    frame_test_name = "/mnt/sdc1/upetruzzello/annotation/cross_fold" + fold +"_test_shuffled_r.csv"
    frame_test = pd.read_csv(frame_test_name, names=["Image", "Mask", "Label", "Intensity"])
    train_string = "/mnt/sdc1/upetruzzello/annotation/cross_fold" + fold +"_train_shuffled_r.csv"
    frame = pd.read_csv(train_string, names=["Image", "Mask", "Label", "Intensity"])

    
    train = HEP2Dataset(stringa, root_dir, transform=args.da)
    #valid = HEP2Dataset(val, root_dir, transform=False)
    ...
    for image in images:
        ...
        path_res = "produced_images/produced_mask_cross_val_shuff_5/" + names[f]

        torchvision.utils.save_image(preds, path_res)
        f += 1
        
    #calculate confusion matrix for intensity
    ...
    root_dir_2 = 'produced_images/produced_mask_cross_val_shuff_5/'
    root_dir_fin = 'produced_images/compared_mask_cross_val_shuff_5/'
    ...
    result_list_patch = [ACC_INT, TOTAL_DICE, TOTAL_ACC]
    
    with open('csv_result/patch_result_cross_val_shuff_final.csv', 'a', newline='') as f:
        csv_writer = csv.writer(f, delimiter=';')
        
        if i == 0:
            csv_writer.writerow(["Intensity Classification", "Dice Score", "Segmentation Accuracy"])
    ...
    for ii in range(total_test):
        ...
        if (ii + 1) % args.patchn == 0:
            ...
            for k in reversed(range(args.patchn)):
                ...
                stringa_img = "produced_images/produced_mask_cross_val_shuff_5/" + names[ii - k]
                masks.append(cv2.imread(stringa_img, cv2.IMREAD_GRAYSCALE))
            
            result = patch_reconstruction(mask)
            fin_string = "produced_images/produced_whole_mask_cross_val_shuff_5/" + names[ii].split("_patch")[0] + "_Mask.tif"
    ...
    np.savetxt("csv_result/modified_ground/confusion_matrix_intensity_whole_" + str(fold) + ".csv", int_cm, delimiter=';', fmt='%d')
    #calculate Accuracy for intensity
    diag = 0
    sum = 0
    for ii in range(len(int_cm)):
    ...
    root_dir_2_whole = 'produced_images/produced_whole_mask_cross_val_shuff_5/'
    root_dir_fin_whole = 'produced_images/compared_whole_mask_cross_val_shuff_5/'
    ...
    with open('csv_result/result_whole_cross_val_shuff_dice_and_acc_per_class_5.csv', 'a', newline='') as f2:
    ...
    result_list = [ACC_INT_WHOLE, TOTAL_DICE_WHOLE, TOTAL_ACC_WHOLE]
